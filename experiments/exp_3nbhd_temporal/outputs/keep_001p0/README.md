...
Helmholtz-potential PINN, **1% temporal availability**, gauge OFF.

## Setup
- **Temporal availability:** 1% of timesteps kept per training point
  (24,388 training rows).
- **Through-thickness scale:** Lz = 1 mm (physical ply spacing, rho ~ 85)
- **Gauge loss:** OFF
...
- This is part of the baseline availability sweep (Lz = 1, gauge off); only the data fraction (1%) varies.

## Results (median held-out relative L2)
| setting | median rel-L2 |
|---|---|
| seen (temporal infill at training points) | 25.385 |
| neighborhood (spatial holdout, 15 unseen pts) | 40.986 |
| far (extrapolation, away from all clusters) | 28.503 |

Per-neighborhood (seen / neighborhood spatial): near_source 4.704 / 5.008, in_between 25.385 / 40.986, near_boundary 298.654 / 136.240
