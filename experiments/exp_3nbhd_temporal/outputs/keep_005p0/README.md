...
Helmholtz-potential PINN, **5% temporal availability**, gauge OFF.

## Setup
- **Temporal availability:** 5% of timesteps kept per training point
  (121,197 training rows).
...
- This is part of the baseline availability sweep (Lz = 1, gauge off); only the data fraction (5%) varies.

## Results (median held-out relative L2)
| setting | median rel-L2 |
|---|---|
| seen (temporal infill at training points) | 3.318 |
| neighborhood (spatial holdout, 15 unseen pts) | 5.383 |
| far (extrapolation, away from all clusters) | 4.435 |

Per-neighborhood (seen / neighborhood spatial): near_source 1.060 / 1.077, in_between 3.318 / 5.383, near_boundary 31.354 / 14.028
