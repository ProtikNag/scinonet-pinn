...
Helmholtz-potential PINN, **15% temporal availability**, gauge OFF.

## Setup
- **Temporal availability:** 15% of timesteps kept per training point
  (363,673 training rows).
...
- This is part of the baseline availability sweep (Lz = 1, gauge off); only the data fraction (15%) varies.

## Results (median held-out relative L2)
| setting | median rel-L2 |
|---|---|
| seen (temporal infill at training points) | 0.699 |
| neighborhood (spatial holdout, 15 unseen pts) | 1.188 |
| far (extrapolation, away from all clusters) | 2.560 |

Per-neighborhood (seen / neighborhood spatial): near_source 0.182 / 1.047, in_between 0.699 / 1.122, near_boundary 4.057 / 2.406
